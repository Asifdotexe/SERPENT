...
def test_break_continue_colors():
    ...
    assert (
        f'fillcolor="{break_color}"' in dot_source
        or f"fillcolor={break_color}" in dot_source
    ), f"Break node missing color {break_color} in dot_source"

    print(f"Checking for continue color: {continue_color}")
    assert (
        f'fillcolor="{continue_color}"' in dot_source
        or f"fillcolor={continue_color}" in dot_source
